...
def pumpc(p, pi, q):  # Determines the cost of the pump for a given pump type, performance rating, and flow rate
    pc = pump[p][pi] * q
    return pc


def turbinec(t, pi, q):  # Determines the cost of the turbine for a given turbine type, performance rating,
    # and flow rate
    tc = turbine[t][pi] * q
    return tc
...
# ---------------------------------------------------
if rSide <= 600:
    if pumpIndex == -1 or pipeIndex == -1 or bend1Index == -1 or bend2Index == -1 or turbineIndex == -1 or diameterIndex == -1 or performanceIndex == -1:
